...
//
forever.stop = function (target, format, scriptArguments) {
  var emitter = new events.EventEmitter(),
      processes = getAllProcesses(),
      results = [];
  
  var procs = /(\d+)/.test(target) ? forever.findByIndex(target, processes)
                                   : forever.findByScript(target, processes, scriptArguments);
...
//
forever.findByScript = function (script, processes, scriptArguments) {
  if (processes===null) {
	return null;
  }

  return processes.filter(function (p) { 
    return p.file === script && p.options.join(' ') === scriptArguments.join(' ')
  });
};
